...
> 📖 **Data diversity คืออะไร?** คือ "ความหลากหลายของข้อมูลเทรน" — วัดว่าคำตอบในชุดข้อมูลซ้ำ ๆ กันมากแค่ไหน ถ้าคำตอบส่วนใหญ่ไม่ซ้ำกัน = หลากหลายสูง ถ้าคำตอบเดิม ๆ วนซ้ำเยอะ = หลากหลายต่ำ
...
> 🎓 **อุปมา "ท่องจำ vs เข้าใจ":** ลองนึกถึงนักเรียนสองคนเตรียมสอบ
> - คนแรก **ท่องจำ (memorize)** เฉลยข้อสอบเก่าซ้ำ 400 รอบ — พอเจอโจทย์เดิมทำได้ทันที แต่เจอโจทย์ใหม่ที่ดัดแปลงนิดเดียวก็ทำไม่ได้เลย
> - คนที่สอง **เข้าใจหลักการ (generalize)** จากตัวอย่างหลากหลายแบบ ๆ ละไม่กี่รอบ — จำเฉลยเป๊ะ ๆ ไม่ได้ แต่พอเจอโจทย์ใหม่ก็ประยุกต์ตอบได้
>
> โมเดลก็เหมือนกัน ถ้าป้อนข้อมูลซ้ำมากไป มันจะกลายเป็นนักเรียนคนแรก (ท่องจำ) ซึ่งไม่ใช่สิ่งที่เราต้องการ
...
โมเดลต้องรู้ว่าส่วนไหนคือคำถามของ user ส่วนไหนคือคำตอบของ assistant จึงต้องมี **template** (แม่แบบจัดรูปแบบข้อความ)

> 💬 **อุปมา "บทละคร":** ChatML ก็เหมือนสคริปต์บทละครที่เขียนกำกับไว้ชัดเจนว่า "ตัวละคร user พูดว่า..." และ "ตัวละคร assistant ตอบว่า..." ถ้าเราส่งข้อความดิบ ๆ ให้โมเดลโดยไม่มีป้ายกำกับ มันจะแยกไม่ออกว่าตรงไหนคือคำถามตรงไหนคือคำตอบ token พิเศษอย่าง `<|im_start|>` และ `<|im_end|>` จึงทำหน้าที่เป็น "วงเล็บกำกับฉาก" ที่บอกจุดเริ่มและจุดจบของแต่ละคนพูด
...
> 📖 **ศัพท์ที่ต้องรู้:**
> - **padding (`<pad>`)** = การ "เติมช่องว่าง" ท้ายประโยคสั้นให้ยาวเท่าประโยคอื่นในชุดเดียวกัน เพราะคอมพิวเตอร์ประมวลผลหลายประโยคพร้อมกัน (เป็น batch) ได้เร็วกว่าถ้าทุกประโยคยาวเท่ากัน เหมือนจัดกล่องให้ขนาดเท่ากันก่อนเรียงขึ้นชั้น
> - **BOS / EOS** = Beginning / End Of Sequence คือ "เครื่องหมายจุดเริ่ม" และ "จุดจบ" ของข้อความ ช่วยให้โมเดลรู้ว่าควรหยุดพิมพ์ตรงไหน (เจอ EOS = จบ)
> - **batch** = กลุ่มข้อมูลหลายตัวอย่างที่ป้อนให้โมเดลประมวลผลพร้อมกันในรอบเดียว
...
>
> 🔤 **อุปมา "ทุกอย่างสร้างจากตัวต่อ 256 สี":** คอมพิวเตอร์เก็บทุกอย่าง—ไม่ว่าจะภาษาอังกฤษ ไทย จีน หรืออีโมจิ 🐟—เป็นชุดของ byte ซึ่งมีแค่ 256 ค่า (0–255) ByteLevel BPE เริ่มจากตัวต่อพื้นฐาน 256 สีนี้ เลยการันตีว่า "ไม่มีอักขระไหนที่มันเข้ารหัสไม่ได้" จึงไม่จำเป็นต้องมี token `<unk>` (unknown = ไม่รู้จัก) ไว้รองรับตัวอักษรแปลก ๆ เหมือน tokenizer แบบเก่า
...
>
> 📝 **แปลง่าย ๆ:** "mask ส่วน prompt" คือการบอกโมเดลว่า "ไม่ต้องหัดเดาคำในส่วนคำถาม ให้หัดเดาแค่ส่วนคำตอบพอ" — GuppyLM เลือกไม่ทำแบบนั้น มันให้โมเดลหัดเดาทุกคำ (ทั้งคำถามและคำตอบ) ยกเว้นช่องว่างเติม (padding) ทั้งสองแนวทางถูกต้องทั้งคู่ เพียงแต่เน้นให้โมเดลเรียนคนละจุด
